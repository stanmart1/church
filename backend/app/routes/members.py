...
@router.get("/export")
async def export_members(format: str = 'csv', db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await member_service.export_members(db, format)
